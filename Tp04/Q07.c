...
void registroLog(int key) {
    FILE *arquivo;
    if (key == 0) {
        arquivo = fopen("811197_hashIndireta.txt", "w");
    } else {
        arquivo = fopen("/tmp/811197_hashIndireta.txt", "w");
    }
    if (arquivo == NULL) {
        perror("Erro ao abrir o arquivo.");
        exit(EXIT_FAILURE); 
    }
    fprintf(arquivo, "Matricula: 811197\tTempo de execucao: %.6f segundos\tComparacoes: %d", tempo, comparacoes);
    fclose(arquivo);
}
